...
/**
 * Fallback currency formatter if utility is not yet resolved.
 */
...
/**
 * Fallback category breakdown calculator if utility is not yet resolved.
 */
const DEFAULT_PALETTE = [
    '#10b981', // cosmic emerald 500
    '#34d399', // cosmic mint 400
    '#14b8a6', // cosmic teal 500
    '#06b6d4', // cosmic cyan 500
    '#059669', // cosmic emerald 600
    '#6ee7b7', // cosmic mint 300
    '#0f766e', // cosmic teal 700
    '#047857', // cosmic emerald 700
    '#8b5cf6', // cosmic violet 500
    '#0ea5e9', // cosmic sky 500
];

function fallbackCategoryBreakdown(subscriptions = [], currency = 'BRL') {
    ...
    activeSubs.forEach((sub) => {
        const catName = sub.category?.trim() || 'Geral';
        const price = Number(sub.monthly_equivalent_price ?? (
        ...
        categoryMap[catName].value += price;
        categoryMap[catName].count += 1;
        totalMonthly += price;
    });

    const palette = (typeof COSMIC_PALETTE !== 'undefined' && Array.isArray(COSMIC_PALETTE))
        ? COSMIC_PALETTE
        : DEFAULT_PALETTE;
    ...
    const data = Object.values(categoryMap)
        ...
        .map((item, index) => ({
            ...
            count: item.count,
            percentage: totalMonthly > 0 ? Number(((item.value / totalMonthly) * 100).toFixed(1)) : 0,
    ...
    const { data, totalMonthly } = useMemo(() => {
        ...
        if (typeof calculateCategoryBreakdown === 'function') {
            try {
                ...
                console.warn('Error in calculateCategoryBreakdown, using fallback:', err);
            }
        }
        return fallbackCategoryBreakdown(subscriptions, selectedCurrency);
